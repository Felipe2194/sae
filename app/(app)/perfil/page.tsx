...
export default function PerfilPage() {
  ...
  const [apodo, setApodo] = useState(usuarioActual.apodo ?? "");
  const [color, setColor] = useState(usuarioActual.color ?? COLORES[0].hex);
  const [guardado, setGuardado] = useState(false);

  const iniciales = (apodo || nombre)
    .split(" ")
    .slice(0, 2)
    .map((p) => p[0])
    .join("")
  ...
  function handleGuardar(e: React.FormEvent) {
    e.preventDefault();
    // TODO: conectar a API real con Auth.js session
    setGuardado(true);
  ...
  return (
    <div className="mx-auto max-w-2xl flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Foto y color</CardTitle>
          <CardDescription>
        ...
        <CardContent className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex flex-col gap-0.5">
              <p className="font-semibold text-base">{apodo || nombre.split(" ")[0]}</p>
              <p className="text-muted-foreground text-sm">{usuarioActual.email}</p>
              <Badge variant="secondary" className="mt-1 w-fit text-xs">
                {ROL_LABEL[usuarioActual.rol] ?? usuarioActual.rol}
              </Badge>
            </div>
          </div>
        ...
        <CardContent>
          <form onSubmit={handleGuardar} className="flex flex-col gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="apodo">Apodo / nombre corto</Label>
                <Input
                  id="apodo"
                  value={apodo}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                <Label>Rol</Label>
                <Input
                  value={ROL_LABEL[usuarioActual.rol] ?? usuarioActual.rol}
                  disabled
                  className="h-11 opacity-60"
                />
              </div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <Button type="submit" className="h-11 text-base px-6">
                {guardado ? (
              ...
              {guardado && (
                <p className="text-muted-foreground text-sm">Los cambios se aplicaron correctamente.</p>
              )}
            </div>
          </form>
        </CardContent>
